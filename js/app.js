  ...
  fetch(CONFIG.php.listEndpoint)
    ...
    .then(data => {
      if (!data || data.ok !== true) {
        const reason = data && data.error ? data.error : "Tundmatu viga.";
        setStatus("myFilesStatus", `MyFiles kausta ei õnnestunud lugeda: ${reason}`);
        renderMyFilesFileList([]);
        return;
      }
    ...
    .catch(err => {
      setStatus("myFilesStatus",
        `MyFiles nimekirja ei õnnestunud lugeda (kas php/list_files.php on serveris kättesaadav ja PHP töötab?). Detail: ${err.message}`);
      console.warn(err);
    });
}
